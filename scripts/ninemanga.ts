		...
		try {
			const res = await client.fetch(url, {
				...
				headers: {
					...
					Cookie: 'ninemanga_template_desk=yes',
					...(options.headers || {}),
				},
				timeout: 30000,
			});
...
const scrapeManga = async (url: string) => {
	...
	const root = $('div.manga').first();

	if (!root.length) {
		throw new Error('Cannot find manga root');
	}

	const infoRoot = root.find('div.bookintro').first();

	const title = root.find('h1').first().text().trim() || 'Untitled';

	const coverUrl = absolute(root.find('img').first().attr('src') || '', url);

	const description = infoRoot.find('[itemprop="description"]').first().text().trim() || title;

	const author = infoRoot.find('[itemprop="author"]').first().text().trim() || 'Unknown';

	const rawTags: string[] = [];

	infoRoot.find('[itemprop="genre"] a').each((_, el) => {
	...
	root.find('div.chapterbox ul li')
		.toArray()
		.reverse()
		.forEach((el, index) => {
...
const fetchPageImage = async (pageUrl: string, chapterUrl: string) => {
	...
	const img = $('a.pic_download').attr('href') || $('img.manga_pic').attr('src');

	if (!img) {
		throw new Error(`Page image not found: ${pageUrl}`);
	}
...
const run = async () => {
	...
	console.log('Scraping NineManga...');
